...
TimeSpan TimeSpan::operator+(const TimeSpan& right) { return TimeSpan(_seconds + right._seconds); } // of overloaded add
TimeSpan TimeSpan::operator-(const TimeSpan& right) { return TimeSpan(_seconds - right._seconds); } // of overloaded subtract
...
bool MCP7940_Class::getOscRun() const { return (readByte(MCP7940_RTCWKDAY) >> MCP7940_OSCRUN) & 1; } // of method getOscRun()
...
void MCP7940_Class::writeRegisterBit(const uint8_t reg, const uint8_t b, const bool bitvalue) const {
  /*!
...
bool MCP7940_Class::deviceStart() const {
  ...
  {
    oscillatorStatus = readRegisterBit(MCP7940_RTCWKDAY, MCP7940_OSCRUN); // Wait for oscillator to start
...
bool MCP7940_Class::deviceStop() const {
  ...
  for (uint8_t j = 0; j < 255; j++) { // Loop until changed or overflow
    oscillatorStatus = readRegisterBit(MCP7940_RTCWKDAY, MCP7940_OSCRUN); // Wait for oscillator to stop
    if (!oscillatorStatus) break; // Exit loop on success
...
DateTime MCP7940_Class::now() const {
  ...
  return DateTime(
      bcd2int(readBuffer[6]) + 2000, bcd2int(readBuffer[5] & 0x1F), bcd2int(readBuffer[4] & 0x3F), bcd2int(readBuffer[2] & 0x3F), bcd2int(readBuffer[1] & 0x7F), bcd2int(readBuffer[0] & 0x7F));
} // of method now
...
uint8_t MCP7940_Class::getMFP() const {
  ...
  */
  ...
  } else if ((controlRegister & (1 << MCP7940_ALM0EN)) | (controlRegister & (1 << MCP7940_ALM1EN))) { // One or both alarms enabled, and no square wave output.
...
bool MCP7940_Class::setAlarm(const uint8_t alarmNumber, const uint8_t alarmType, const DateTime& dt, const bool state) const {
  /*!
